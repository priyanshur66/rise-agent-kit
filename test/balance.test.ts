...
// Test ERC20 balance via natural language
await test('get ERC20 balance via natural language', async () => {
  const result = await agent.execute(
    `Check the ERC20 token balance of your own wallet for contract address ${ERC20_TEST_TOKEN_ADDRESS}.`
  );
  console.log('Agent response:', result);
});
...
// Test ERC20 transfer via natural language
await test('transfer ERC20 via natural language', async () => {
  const result = await agent.execute(
    `Transfer 0.01 tokens of the ERC20 at contract address ${ERC20_TEST_TOKEN_ADDRESS} to wallet address 0x936cEfb89d47F620EAb665D9Bd27BA06b0cF11c7.`
  );
  console.log('Agent response:', result);
});
